...
class NFMarkdowner:
    ...
    def download_contents(self, begin=None, end=None):
        ...
        for chap in self.chaps:
            ...
            data = soup.find('div', attrs={"class":u"cha-tit"})
            chap_name = ''
            #if data.find('strong'):
            ...
            #    data.b.extract()
            if data and data.find('h3'):
                print('title from h3')
                print(data.find('h3'))
                chap_name = data.find('h3').text.strip()
                data.h3.extract()
            else:
                ...
                elif data and data.find('p').text.startswith("Chapter"):
            ...
            # if there is such things we need to get ride of &s and #s
            data_str = str(data)
            if data_str.find("&amp;"):
                print("We got &amp;s")
    ...
    def get_chapter_from_index(self, file_name, starts=["Chapter"], nostarts=["Chapters"], last_cmpt=1):
        ...
        for link in data:
            if self.chap_limit > 0 and cmpt > self.chap_limit:
                break
            if (not link.parent.parent.has_attr('class') or (link.parent.parent.has_attr('class') and not "list-chapter" in link.parent.parent['class'])):
                # This is not a chapter link
                #print("Skipped not good class:", link.text.strip())
                continue
            ...
            if (link.text.strip().startswith(tuple(starts)) and
                ...
                self.chaps.append((cmpt, 'https://novelfull.com' + link['href'], link.text.strip()))
                cmpt += 1
            else:
                #print("Skipped:", link.text.strip())
                continue

        # This is special for Novel Full indexes...
        data = soup.find("li", attrs={"class":"next"})
        if data.has_attr('class') and not "disabled" in data['class']:
            # We should get the next index file and then parse it
            tmp_data = data.find('a')
            turl = 'https://novelfull.com' + tmp_data['href']
            file_name = "tmp_index"
            download_file(turl, file_name)
            self.get_chapter_from_index(file_name, starts, nostarts, cmpt)
...
if __name__ == "__main__":
    ...
    MYMARKDOWNER = NFMarkdowner("MartialGodAsura", "Martial God Asura",
